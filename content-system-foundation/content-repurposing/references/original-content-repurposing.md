> 原始文件：`content-repurposing/SKILL.md`（来源 rampstackco/claude-skills，MIT，24KB 完整方法论）。
> 本文件为原始全文，未改写。本 Skill 的 WorkBuddy 适配见 `../SKILL.md`。
> 说明：原始 9 个 references 中，本安装未收录 `aeo-extraction-patterns.md`（AI 搜索/SEO 提取），
> 因个人短视频 IP 场景下你明确要求「不为 SEO/流量强行塞关键词」，该 reference 超范围。其余 8 个原样保留。
